...
import apiReferencesData from '@/data/mock/api.json'
import changelogsData from '@/data/mock/changelog.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'api.json': apiReferencesData,
  'changelog.json': changelogsData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetApiReferences') || query.includes('nodeApiReferences')) {
      return loadMockData('api.json')
    }

    if (query.includes('GetChangelogs') || query.includes('nodeChangelogs')) {
      return loadMockData('changelog.json')
    }
