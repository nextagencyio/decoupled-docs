...
function PageNotFound({ path }: { path: string }) {
  return (
    <div className="text-center py-16">
      <h1 className="font-mono text-2xl font-semibold text-primary-900 mb-4">Page Not Found</h1>
      <p className="text-primary-400 mb-2">We couldn&#39;t find any content at this path.</p>
      <p className="text-sm text-primary-300">Path: {path}</p>
    </div>
  )
}
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-16 md:pt-48 md:pb-24">
          <ErrorBoundary>
            <article>
              ...
              <h1 className="font-mono text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-primary-950 leading-[0.95] mb-8">{title}</h1>
              {bodyHtml && (
                <div className="prose prose-lg max-w-none prose-headings:font-mono prose-headings:tracking-tight prose-a:text-accent-500" dangerouslySetInnerHTML={{ __html: bodyHtml }} />
              )}
            </article>
          </ErrorBoundary>
        </main>
